...
export const createPet = async (req, res) => {
  try {
    ...
    const mainImage = await uploadImage(req.body.mainImage, "pets");

    if (!name || !type || !breed || !age || !gender || !location || !aboutText || !mainImage) {
      return res.status(400).json({
        message: "Please fill all required fields: name, type, breed, age, gender, location, description, and image.",
      });
    }
